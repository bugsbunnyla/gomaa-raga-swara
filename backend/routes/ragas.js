...
router.get('/lookup', async (req, res) => {
  try {
    const q = (req.query.q || '').trim().toLowerCase();const express = require('express');
const router = express.Router();
const path = require('path');
const fs = require('fs');
const sqliteModule = require('../../core/db/sqlite');
...
// GET /api/ragas — Return full raga list (enriched with janyas)
router.get('/', async (req, res) => {
  try {
    const ragas = getRagaCache();
    const enriched = ragas.map(r => {
      const janyas = ragas.filter(j => j.parent === r.name).map(j => j.name);
      return {
        ...r,
        janyas: janyas.length ? janyas : null,
        arohana: r.arohana || (r.aroha ? r.aroha.split(/\s+/) : []),
        avarohana: r.avarohana || (r.avaroha ? r.avaroha.split(/\s+/) : [])
      };
    });
    res.json(enriched);
  } catch (err) {
    console.error('[ragas]', err.message);
