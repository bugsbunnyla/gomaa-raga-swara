...
/**
 * generateScoreXML — Wrapper for recognize.js compatibility.
 * Signature: generateScoreXML(raga, tala, section, swaraResult, opts)
 */
function generateScoreXML(raga, tala, section, swaraResult, opts = {}) {
  const result = buildMusicXML({
    ...
    parent: opts.parent || "",
    tala: tala || "Adi",
    beatsPerCycle: opts.beatsPerCycle || 8,
    bpm: opts.bpm || 120,
    aroha: opts.aroha || "",
    avaroha: opts.avaroha || "",
    swaras: swaraResult || {},
    sahityam: opts.sahityam || {},
    duration: opts.duration || 120
  });
  return result.xml || "";
}

module.exports = { buildMusicXML, buildMIDI, generateScoreXML, swaraToWestern };